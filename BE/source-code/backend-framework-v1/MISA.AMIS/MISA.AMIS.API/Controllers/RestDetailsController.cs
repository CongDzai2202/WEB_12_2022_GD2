    ...
    public class RestDetailsController : BasesController<RestDetail>
    {
            ...
            )
        {
            try
            {
                ...
                result = _RestDetailBL.GetRestDetailsByFilter(keyword, statusId, departmentName, filterParams, pageSize, pageNumber);
        ...
        [HttpPost("export")]
        public async Task<IActionResult> ExportToExcel(CancellationToken cancellationToken, List<Guid>? listOverTimeId)
        {
            await Task.Yield();
            var stream = _RestDetailBL.ExportToExcel(listOverTimeId);
