...
namespace MISA.AMIS.BL.RestDetailBL
{
    public class RestDetailBL : BaseBL<RestDetail>, IRestDetailBL
    {
...

        {
            ...
            foreach (var item in filterParams)
            {
                if (item.DataType == "date")
                {
                    if (item.Operator == "%")
                    {
                        sql = String.Format("CAST(DATE_FORMAT({0}, '%Y/%m/%d') AS char) LIKE '{1}{2}{1}' AND ", item.Field, item.Operator, item.Value);
                    }
                    else if (item.Operator == "!%")
                    {
                        sql = String.Format("CAST(DATE_FORMAT({0}, '%Y/%m/%d') AS char) NOT LIKE '%{1}%' AND ", item.Field, item.Value);
                    }
                    else
                    {
                ...
                else
                {
                    if (item.Operator == "%")
                    {
                        sql = String.Format("{0} LIKE '{1}{2}{1}' AND ", item.Field, item.Operator, item.Value);
                    }
                    else if (item.Operator == "!%")
                    {
            ...
            result = _restDetailDL.GetRestDetailsByFilter(keyword, statusId, departmentName, queryString, pageSize, pageNumber);
        ...
        private static void FomatStyleForExcel(List<RestDetail> listRestDetails, ExcelWorksheet workSheet)
        {
            ...
            workSheet.Cells["A:S"].Style.Font.SetFromFont("Time New Roman", 12);
